...
> Imported from [aaddrick/claude-desktop-debian](https://github.com/aaddrick/claude-desktop-debian).
> The same patching strategy applies to claude-desktop-suse — the
> minified upstream JavaScript is identical regardless of which Linux
> packaging fork it lands in.

The verification recipes below use the same incantations
(Claude-Setup.exe, nupkg extraction, `build.sh --build appimage`)
that the SUSE build uses; the only difference is the output package
format (`.rpm` instead of `.deb`).
